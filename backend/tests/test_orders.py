...
def test_create_order(client, db_session):
    ...
    assert data["green_credits_earned"] == 0 # express delivery
...
def test_create_order_eco_delivery(client, db_session):
    ...
    response = client.post("/api/orders/", json={
        ...
        "product_id": 1,
        "price": 100.0,
        "delivery_type": "eco", # This should trigger eco bonus
        "is_community": False
    })
    assert response.status_code == 201
    data = response.json()
...
def test_vest_credits_too_early(client, db_session):
    # Create an order
    order_res = client.post("/api/orders/", json={"user_id": 1, "product_id": 1, "price": 100.0, "delivery_type": "express"})
    order_id = order_res.json()["id"]

    # Try to vest immediately (window should still be open)
    response = client.post(f"/api/orders/{order_id}/vest-credits")
