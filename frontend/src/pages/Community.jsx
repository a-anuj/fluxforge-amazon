...
import {
  ...
  markNotificationsRead, createAlert
} from "../api/client";
...
export default function Community() {
  ...
  const fetchListings = async () => {
    ...
    setLoading(true);
    try {
      const data = tab === "nearby"
        ? await getNearbyListings(currentUser.id)
        : await getCommunityListings(currentUser.id);
      setListings(data);
    } catch(e) { console.error(e); }
    finally { setLoading(false); }
  };

  useEffect(() => { if (tab !== "leaderboard") fetchListings(); else setLoading(false); }, [tab, currentUser]);

  useEffect(() => {
    if (tab === "leaderboard") getLeaderboard().then(setLeaderboard).catch(console.error);
  }, [tab]);
  ...
  return (
    <div className="bg-amazon-bg min-h-screen animate-fade-in">
      ...
      <div className="bg-gradient-to-r from-[#0f2027] via-[#203a43] to-[#2c5364]">
        <div className="max-w-[1500px] mx-auto px-4 py-8">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[#00e5a0] text-[13px] font-semibold uppercase tracking-widest mb-2">♻️ Community Resale</p>
              <h1 className="text-[32px] md:text-[40px] font-bold text-white leading-tight mb-2">
                Sell What You Own.<br/>
                <span className="text-[#00e5a0]">Save the Planet.</span>
              </h1>
              <p className="text-[#aaa] text-[14px] max-w-lg mb-5">
                List your unused items, earn Green Credits, and keep e-waste out of landfills.
                Local buyers get priority — zero delivery, maximum impact.
              </p>
              <button
          ...
          <div className="grid grid-cols-3 gap-4 mt-6">
            {[
              { label: "E-waste Prevented", val: `${listings.reduce((s,l) => s + (l.ewaste_kg_saved||0), 0).toFixed(1)} kg`, icon: "🌍" },
              { label: "Active Listings", val: listings.filter(l => l.status === "active").length, icon: "📦" },
              { label: "Near You", val: listings.filter(l => l.is_local).length, icon: "📍" },
            ].map(s => (
      ...
      <div className="max-w-[1500px] mx-auto px-4 py-4">
        <div className="flex gap-0 border-b border-amazon-border mb-4 bg-white px-4 rounded-t-lg">
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`text-[14px] px-5 py-3 border-b-2 transition-colors ${
        ...
        {tab !== "leaderboard" && (
          loading ? (
            ...
            <div className="bg-white border border-amazon-border rounded-lg p-12 text-center">
              ...
              <p className="text-[16px] font-semibold mb-1">No listings {tab === "nearby" ? "near you" : "yet"}</p>
              <p className="text-[13px] text-amazon-text-secondary mb-4">
                {tab === "nearby" ? "Make sure your pincode is set in your profile." : "Be the first to post!"}
              </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {listings.map(listing => (
                <div key={listing.id} className="bg-white border border-amazon-border rounded-lg overflow-hidden hover:shadow-lg transition-shadow flex flex-col">
                  {/* Image */}
                  <div className="h-[160px] bg-[#f5f5f5] flex items-center justify-center relative">
                    {listing.image_urls ? (
                      <img
                  ...
                  <div className="p-3 flex flex-col flex-1">
                    ...
                    <div className="mt-auto">
                      <div className="flex items-baseline gap-2 mb-1">
                        <span className="text-[20px] font-bold text-amazon-text">
                        ...
                        {listing.suggested_price && listing.suggested_price !== listing.asking_price && (
                          <span className="text-[11px] text-amazon-text-secondary">AI: ₹{Math.floor(listing.suggested_price).toLocaleString("en-IN")}</span>
                        )}
                      </div>
                      ...
                      {listing.status === "active" && currentUser && listing.seller_id !== currentUser.id ? (
                        <button
                          ...
                          className="w-full bg-amazon-orange hover:bg-amazon-orange-hover text-amazon-text font-bold text-[13px] py-2 rounded transition-colors"
                        >
                          {buying === listing.id ? "Processing…" : "Buy Now"}
                        </button>
                      ) : listing.status === "sold" ? (
                        <span className="block w-full text-center text-[13px] text-amazon-text-secondary bg-[#f0f2f2] py-2 rounded">Sold</span>
                      ) : listing.seller_id === currentUser?.id ? (
                        <span className="block w-full text-center text-[13px] text-[#00a86b] bg-[#f0fff8] py-2 rounded font-semibold">Your Listing</span>
                      ) : null}
                    </div>
                  </div>
                </div>
      ...
      {/* Post Listing Modal */}
      {showModal && <PostModal currentUser={currentUser} onClose={() => setShowModal(false)} onSuccess={() => { setShowModal(false); fetchListings(); showToast("✅ Listing posted! +5 Green Credits earned."); }} />}
